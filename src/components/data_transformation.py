...
class DataTransformation:
    ...
    def get_data_transformer_object(self, numerical_columns, categorical_columns):
        '''
        This function is responsible for data transformation using dynamically identified columns.
        '''
        try:
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            # Ensure target column is excluded before identifying input features
            input_features = train_df.drop(columns=[target_column_name], axis=1)

            # Identify Numerical and Categorical columns dynamically (excluding target)
            numerical_columns = input_features.select_dtypes(exclude=['object']).columns.tolist()
